...
// import { FaAmbulance, FaBell, FaLocationArrow, FaRoute } from 'react-icons/fa'; // Removed FaFire
// import dynamic from 'next/dynamic';
// import { parseStringPromise } from 'xml2js';
// import AlertCard from './component/AlertCard';
...
//       const parsed = await parseStringPromise(result.data);
//       const items = parsed.rss.channel[0].item || [];
//       const newAlerts = items.map((item: GdacsItem, index: number) => {
//         const title = item.title[0].toLowerCase();
//         const lat = parseFloat(item['geo:lat']?.[0] || '0');
//         const lng = parseFloat(item['geo:long']?.[0] || '0');
//         return {
...
//       setFetchError(error instanceof Error ? error.message : 'Unknown error fetching GDACS data');
//     }
//   }, [alerts.length, notificationsEnabled, userLocation]);

//   useEffect(() => {
//     fetchGdacsAlerts();
//   }, [fetchGdacsAlerts]);

//   const updateAlertDistances = useCallback((coords: { lat: number; lng: number }) => {
//     setAlerts((prevAlerts) =>
//       prevAlerts.map((alert) => ({
//         ...alert,
//         distance: calculateDistance(coords.lat, coords.lng, alert.coords.lat, alert.coords.lng),
...
//       updateAlertDistances(userLocation);
//     }
//   }, [userLocation, selectedAlert, safeRouteLocation.lat, safeRouteLocation.lng, updateAlertDistances]);
